...
function validatePrivKey(privKey) {
	if (!(privKey instanceof Buffer) || privKey?.byteLength !== 32) return false;
	return true;
}

function scrubPubKeyFormat(pubKey) {
	if (!(pubKey instanceof Buffer)) return false;
...
exports.generateKeyPair = function () {
	if (typeof nodeCrypto.generateKeyPairSync === 'function') {
		...
		return { privKey: Buffer.from(keyPair.private), pubKey: Buffer.from(keyPair.public) };
	}
};

exports.calculateAgreement = function (pubKey, privKey) {
	pubKey = scrubPubKeyFormat(pubKey);
	if (!validatePrivKey(privKey) || !pubKey || pubKey.byteLength !== 32) return false;
...
exports.calculateSignature = function (privKey, message) {
	if (!validatePrivKey(privKey) || !message) return false;
	return Buffer.from(curveJs.sign(privKey, message));
};

exports.verifySignature = function (pubKey, msg, sig) {
	pubKey = scrubPubKeyFormat(pubKey);
	if (!pubKey || pubKey.byteLength !== 32 || !msg || !sig || sig.byteLength !== 64) return false;
